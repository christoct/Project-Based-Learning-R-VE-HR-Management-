...
<script src="hapus_pelamar.js"></script>
...
1.5 0 0 0 1.5 1.5h6a1.5 1.5 0 0 0 1.5-1.5V15a.75.75 0 0 1 1.5 0v3.75a3
    ...
    <div class="bar"> <span class="new" id="new_pelamar">Baru</span>
        <span class="tittle_menu">Pelamar</span>
...
5.196a7.5 7.5 0 0 0 10.607 10.607Z" />
    ...
    <form action="proses_hapus_pelamar.php" id="form_hapus_pelamar" method="post">
        <div class="popup" id="hapus_pelamar_popup">
            <span class="x">&times;</span>
            <input type="hidden" name="id_pelamar_hapus" id="id_pelamar_hapus">
            <span class="popup_tittle">Hapus Pelamar</span>
            <hr class="line">
            <span class="popup_text" id="nama_pelamar_hapus">Apakah Anda yakin mau menghapus pelamar ini?</span>
            <br>
            <div class="popup_buttons">
                <span class="ya" id="confirm_hapus_pelamar">Ya</span>
                <span class="tidak">Tidak</span>
            </div>
        </div>
    </form>

    <div class="wadah_pelamar">
        <?php
        include("config.php");
        ...
        if (isset($_GET['search']) && !empty(trim($_GET['search']))) {
            ...
            $sql = "SELECT pelamar.*, lowongan.nama_lowongan, status_lamaran.nama_status_lamaran
        FROM pelamar
        JOIN lowongan ON pelamar.id_lowongan = lowongan.id_lowongan
        JOIN status_lamaran ON pelamar.id_status_lamaran = status_lamaran.id_status_lamaran
        WHERE pelamar.nama_pelamar LIKE '%$keyword%'";
        } else {
            $sql = "SELECT pelamar.*, lowongan.nama_lowongan, status_lamaran.nama_status_lamaran
        FROM pelamar
        JOIN lowongan ON pelamar.id_lowongan = lowongan.id_lowongan
        ...
        if ($result->num_rows > 0) {
            while ($row = $result->fetch_assoc()) {
                echo ("<div class='data_pelamar'>
            <span class='tittle_lowongan'>" . $row["nama_pelamar"] . "</span><br>
            <hr>
            <span class='body_pelamar'>" . $row["nama_lowongan"] . "</span>
            <span class='pelamar_stat'>" . $row["nama_status_lamaran"] . "</span><br>
            <hr>
            <div class='ubah_rekrutmen' id=''>Ubah</div>
            <div class='hapus_rekrutmen'>Hapus</div>
            <form action='update_pelamar.php' class='update_pelamar' method='POST'>
                <input type='hidden' class='id_pelamar' name='id_pelamar' value='" . $row["id_pelamar"] . "'>
                <input type='hidden' class='nama_pelamar' name='nama_pelamar' value='" . $row["nama_pelamar"] . "'>
            </form>
